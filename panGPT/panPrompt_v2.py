...
def print_banner():
    ...
    *        panPrompt v0.02 (with top-k/top-p)      *
...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model, max_len=5000, dropout=0.1):
        ...
        # Create positional encoding with shape [max_len, 1, d_model] to match checkpoint
...
def predict_next_tokens(model, tokenizer, prompt, num_tokens, temperature=1.0, top_k=50, top_p=0.95):
    """
    Improved token prediction with top-k and top-p (nucleus) sampling
...
def main():
    print_banner()
    parser = argparse.ArgumentParser(description="Token prediction with top-k and top-p sampling.")
    ...
    if args.model_type == 'transformer':
        model = SimpleTransformerModel(
            ...
            pe_max_len=5000,  # Match checkpoint
